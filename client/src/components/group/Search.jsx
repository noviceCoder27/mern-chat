...
function Search({ type, isLoading, searchResults, handleClick, search }) {
  ...
  return (
    <div className={`${search ? "scrollbar-hide overflow-y-scroll h-[250px] mb-5 bg-[#4d426d] flex flex-col gap-y-3 pt-3" : "hidden"}`}>
              ...
              <div key={e._id} className='flex items-center justify-between'>
                <div className='flex items-center gap-x-2'>
                  ...
                  <div className='flex flex-col gap-y-[1px]'>
                    <h5 className='text-[15px] text-white tracking-wide font-medium'>{e.name}</h5>
                    <h5 className='text-[12px] text-[#efaa86] tracking-wide font-normal'>{e.email}</h5>
                  </div>
                </div>
                <button onClick={() => handleClick(e)} className='bg-[#0086ea] px-3 py-2 text-[10.6px] tracking-wide text-[#fff]'>Add</button>
              </div>
            )
          }) : <span className='text-[13px] text-white'>No results found</span>
        )
