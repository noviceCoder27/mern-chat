...
function Group() {
  ...
  const handleSubmit = async () => {
    if (selectedUser.length >= 2) {
      ...
      toast.error("Minimum two members required");
    }
  }
  useEffect(() => {
    ...
    <>
      <button className='mt-1 transition duration-150 ease-in-out' onClick={handleOpen}>
        ...
        <div className='flex justify-start my-3 '>
          <button className='text-[11px] font-normal tracking-wide  rounded-md hover:bg-[#f0a279] flex items-center gap-x-1 bg-[#efaa86] text-[#1f2228] py-1 -mb-7 mt-2  px-2'>New Group <BsPlusLg /></button>
        </div>
      </button>
      <Modal
        open={open}
      ...
      >
        <Box sx={style}>
          ...
          <form onSubmit={(e) => e.preventDefault()} className='flex flex-col mt-3 gap-y-3'>
